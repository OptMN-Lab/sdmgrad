...
class SDMGrad(WeightMethod):
    def __init__(self, n_tasks: int, device: torch.device, lamda=0.6, niter=20, max_norm=1):
        super().__init__(n_tasks, device, max_norm)
        self.lamda = lamda
        self.niter = niter
        self.max_norm = max_norm
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        for param in shared_parameters:
            grad_dims.append(param.data.numel())
        grads = [torch.Tensor(sum(grad_dims), self.n_tasks).to(self.device)] * 3

        for i in range(self.n_tasks * 3):
            if i < self.n_tasks * 3 - 1:
                losses[i].backward(retain_graph=True)
            else:
                losses[i].backward()
    ...
    def sdmgrad(self, w, grads, lamda=0.3, niter=20):
        ...
        GG = torch.mm(xi_grads1.t(), xi_grads2)
